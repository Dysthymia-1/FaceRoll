...
#Version 1.7 to 1.8 - Fixed a Minutia folder variable entry in the line creating the array item folder on the destination drive.
#Version 1.8 to 1.9 - Updated the current time before it's written for each backup log entry.
...
$DriveInfo=  Get-Volume | Select -Property DriveLetter, Size, SizeRemaining | Where-Object {$_.DriveLetter -ne $Null} | Sort-Object DriveLetter
ForEach ($Object in $DriveInfo) { $Object.Size /= (1024*1024*1024)} 
ForEach ($Object in $DriveInfo) { $Object.Size = [math]::Round($Object.Size,0)} 
ForEach ($Object in $DriveInfo) { $Object.SizeRemaining /= (1024*1024*1024)} 
ForEach ($Object in $DriveInfo) { $Object.SizeRemaining = [math]::Round($Object.SizeRemaining,0)} 
...
Add-Content "$LogFile" "Backing up $DataSize GB from $NumDrives drive[s] to the $BDrive drive with a capacity of $BackupCapacity GB."
...
New-Item -Path $MinutiaBackupPath -Name $MinutiaFolder -ItemType "Directory"
